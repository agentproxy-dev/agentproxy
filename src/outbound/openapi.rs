...
impl Handler {
  ...
  /// Path params need to be added to the template params in the path.
	...
	pub async fn call_tool(&self, name: &str, args: Option<JsonObject>) -> Result<String, anyhow::Error> {
        ...
        let path_params = args.get(&*PATH_NAME).and_then(Value::as_object).cloned().unwrap_or_default();
        let query_params = args.get(&*QUERY_NAME).and_then(Value::as_object).cloned().unwrap_or_default();
        let header_params = args.get(&*HEADER_NAME).and_then(Value::as_object).cloned().unwrap_or_default();
        ...
        for (key, value) in &path_params {
            ...
            if let Some(s_val) = value.as_str() {
                 ...
                 tracing::warn!("Path parameter '{}' for tool '{}' is not a string (value: {:?}), skipping substitution", key, name, value);
            }
        }
        ...
        let query_params_reqwest: Vec<(String, String)> = query_params
            ...
            .filter_map(|(k, v)| {
                match v.as_str() {
                    ...
                    None => {
                         tracing::warn!("Query parameter '{}' for tool '{}' is not a string (value: {:?}), skipping", k, name, v);
                         None
                    }
                }
            })
        ...
        for (key, value) in &header_params {
             if let Some(s_val) = value.as_str() { // Headers must be strings.
                 match (HeaderName::from_bytes(key.as_bytes()), HeaderValue::from_str(s_val)) {
                    (Ok(h_name), Ok(h_value)) => {
                        headers.insert(h_name, h_value);
                    },
                    (Err(_), _) => tracing::warn!("Invalid header name '{}' for tool '{}', skipping", key, name),
                    (_, Err(_)) => tracing::warn!("Invalid header value '{}' for header '{}' in tool '{}', skipping", s_val, key, name),
                 }
             } else {
                 tracing::warn!("Header parameter '{}' for tool '{}' is not a string (value: {:?}), skipping", key, name, value);
             }
        }

        // --- Request Building ---
        // Parse the HTTP method.
		let method = Method::from_bytes(info.method.as_bytes())
        ...
        if status.is_success() {
            ...
            Err(anyhow::anyhow!("Upstream API call for tool '{}' failed with status {}: {}", name, status, response_text))
        }
	}
...
mod tests {
    ...
    use std::collections::HashMap;
    use std::sync::Arc;
    use wiremock::matchers::{method, path, path_regex, query_param, header, body_json};
    ...
    async fn setup() -> (MockServer, Handler) {
        ...
        let test_tool_get = Tool {
            ...
            input_schema: Arc::new(json!({ // Define a simple schema for testing
                "type": "object",
                "properties": {
                    "path": {
                        "type": "object",
        ...
        let test_tool_post = Tool {
            ...
            input_schema: Arc::new(json!({ // Schema for POST
                "type": "object",
                "properties": {
                    "body": {
                        "type": "object",
        ...
        let handler = Handler {
            host,
            client,
            tools: vec![(test_tool_get, upstream_call_get), (test_tool_post, upstream_call_post)],
        };
    ...
    async fn test_call_tool_get_simple_success() {
        ...
        let result = handler.call_tool("get_user", Some(args.as_object().unwrap().clone())).await;
    ...
    async fn test_call_tool_get_with_query() {
        ...
        let expected_response = json!({ "id": user_id, "name": "Test User", "details": "Verbose details" });
    ...
    async fn test_call_tool_post_with_body() {
        ...
        let result = handler.call_tool("create_user", Some(args.as_object().unwrap().clone())).await;
    ...
    async fn test_call_tool_tool_not_found() {
        ...
        let args = json!({});
        let result = handler.call_tool("nonexistent_tool", Some(args.as_object().unwrap().clone())).await;
    ...
    async fn test_call_tool_invalid_path_param_value() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(404) // Or whatever the server does with a literal {user_id}
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("failed with status 404 Not Found"));
