...
import { PlusCircle, Loader2, Info, Edit2, HelpCircle } from "lucide-react";
import { Target, Config } from "@/lib/types";
import { updateTarget, createMcpTarget, createA2aTarget, deleteA2aTarget, deleteMcpTarget } from "@/lib/api";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export const TargetsConfig = forwardRef<
...
>(({
  ...
  const [targetToDelete, setTargetToDelete] = useState<{index: number, name: string} | null>(null);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.error("Error deleting target", {
        description: err instanceof Error ? err.message : "Failed to delete target. The target may be in use or the server encountered an error.",
      });
    } finally {
    ...
    <div>
      ...
      {config?.targets && config.targets.length > 0 ? (
        ...
        <Alert>
          ...
          <AlertDescription>
            No targets configured yet. Click &apos;Add Target&apos; to create your first
            target.
          </AlertDescription>
        </Alert>
      )}
        ...
        <DialogContent className="sm:max-w-[600px]">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              />
              {targetNameError && (
                <p className="text-sm text-red-500">{targetNameError}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="max-w-xs">Choose between MCP for AI model servers or A2A for agent-to-agent communication</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              ...
              <RadioGroup
                ...
                onValueChange={(value) => {
                  ...
                  if (editingTarget && ((value === "a2a" && !editingTarget.a2a) || (value === "mcp" && editingTarget.a2a))) {
                    setEditingTarget(undefined);
                  }
                }}
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="flex items-center space-x-2">
                          ...
                          <Label htmlFor="mcp" className="cursor-pointer">MCP Target</Label>
                        </div>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p className="max-w-xs">MCP (Model Control Protocol) targets are used to connect to AI model servers that support the MCP protocol. These are typically used for AI model inference and control.</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="flex items-center space-x-2">
                          ...
                          <Label htmlFor="a2a" className="cursor-pointer">A2A Target</Label>
                        </div>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p className="max-w-xs">A2A (Agent-to-Agent) targets are used to connect to other agent systems that support the A2A protocol. These are typically used for agent-to-agent communication and collaboration.</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
